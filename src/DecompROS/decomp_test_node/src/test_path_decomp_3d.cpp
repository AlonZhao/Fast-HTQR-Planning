...
int main(int argc, char ** argv){
  ...
  nav_msgs::Path path_msg = DecompROS::vec_to_path(path);//路径点到显示路径 可以不用画出来
  ...
  decomp_util.set_local_bbox(Vec3f(1, 2, 1));//算法中为了提高效率定义的初始最大box
  ...
  for(size_t i = 0; i < path.size() - 1; i++) {
    const auto pt_inside = (path[i] + path[i+1]) / 2;//通过已经在圆心的点  判断路径点是否在内部
