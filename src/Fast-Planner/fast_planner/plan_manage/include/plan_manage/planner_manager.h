...
public:
  ...
  void initPlanModules(ros::NodeHandle& nh);//kino路径搜索以及Bspline Opti的初始化
  void setGlobalWaypoints(vector<Eigen::Vector3d>& waypoints);

  bool checkTrajCollision(double& distance);//计算规划出的轨迹距离障碍的距离 小于阈值就返回碰撞之前的安全距离

  PlanParameters pp_;
  LocalTrajData local_data_;
  GlobalTrajData global_data_;
  MidPlanData plan_data_;//存放数据
...
public:
  typedef unique_ptr<FastPlannerManager> Ptr;//本类型的指针
