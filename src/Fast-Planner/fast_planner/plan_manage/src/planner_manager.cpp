...
void FastPlannerManager::initPlanModules(ros::NodeHandle& nh) {
  /* read algorithm parameters 路径搜索与B样条的初始化*/
...
bool FastPlannerManager::checkTrajCollision(double& distance) {
  ...
  Eigen::Vector3d cur_pt = local_data_.position_traj_.evaluateDeBoor(tm + t_now);//计算轨迹上每个离散的点的数值
  ...
  while (radius < 6.0 && t_now + fut_t < local_data_.duration_) {
    ...
    double dist = edt_environment_->evaluateCoarseEDT(fut_pt, -1.0);//计算距离
    if (dist < 0.1) {
      distance = radius;
      return false;
    }
...
std::cout<<"### ctrl_pts size "<<ctrl_pts.size()<<std::endl;
  ...
  if (status != KinodynamicAstar::REACH_END) {//没到达终点的时候要加上END 优化项 到达的花不需要 不包含Pb个控制点
