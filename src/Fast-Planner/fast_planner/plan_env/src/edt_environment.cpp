...
void EDTEnvironment::getSurroundDistance(Eigen::Vector3d pts[2][2][2], double dists[2][2][2]) {
  for (int x = 0; x < 2; x++) {
    for (int y = 0; y < 2; y++) {
      for (int z = 0; z < 2; z++) {
        dists[x][y][z] = sdf_map_->getDistance(pts[x][y][z]);//返回周围六个点的-最近障碍物的距离
      }
    }
  }
}
...
void EDTEnvironment::evaluateEDTWithGrad(const Eigen::Vector3d& pos,
                                                                  ...
                                                                  Eigen::Vector3d& grad) {
  Eigen::Vector3d diff;//三个方向的
  Eigen::Vector3d sur_pts[2][2][2];//上下左右的相邻点的位置
  sdf_map_->getSurroundPts(pos, sur_pts, diff);//获得一个点周围的点存放在sur_pts 的位置和该点的梯度

  double dists[2][2][2];
  getSurroundDistance(sur_pts, dists);//输入sur_pts 获得他的最近距离
  //==>dist grad
  interpolateTrilinear(dists, diff, dist, grad);//三维线形插值 通过四周点最近障碍物 获得当前点的梯度
}
